...
sum(amount) over(partition by customer_id, amount rows unbounded preceding) as Нарастающий_итог --Тут с помощью агрегирующей функции sum считаем суммы платежей и с помощью партиции подсчитываем для каждого покупателя по отдельности строки amount неограниченно суммируя предыдущие значения.
from payment p
...
row_number() over(partition by customer_id) as Номер_платежа_покупателя, --Нумеруем платежи для каждого покупателя, у них отдельные нумерованные списки платежей отсортированные по дате.
amount as Сумма_платежа, --Теперь для удобства выводим сумму отдельного платежа
dense_rank() over (order by amount desc) as Номер_платежа_по_стоимости, --Здесь с помощью функции dense_rank проставляем номера платежам, но не по отдельности для каждого покупателя по стоимости от наибольших к меньшим, но все ещё по отдельности. Dense потому что обычный rank пропускал бы значения после повторений.
sum(amount) over(partition by customer_id, amount rows unbounded preceding) as Нарастающий_итог --Тут с помощью агрегирующей функции sum считаем суммы платежей и с помощью партиции подсчитываем для каждого покупателя по отдельности строки amount неограниченно суммируя предыдущие значения.
from payment p 
order by payment_date, amount ;
...
with RankedPayments as (
	select 
	*,
	row_number () over (partition by customer_id order by payment_date desc) as rn --И тут как то некрасиво
	from payment p 
)
select 
*
